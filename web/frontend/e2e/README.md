...
- **Credenciais do dashboard** disponíveis como `E2E_EMAIL` + `E2E_PASSWORD` (auto-login). Em launchers legados que aceitam senha única, só `E2E_PASSWORD` basta. Se a senha não estiver setada e o launcher exigir auth, os testes pousam na tela `/launcher-login` e falham.
...
E2E_EMAIL='owner@example.com' E2E_PASSWORD='senha-do-dashboard' pnpm test:e2e

# Inclui specs destrutivos (criam/editam/deletam agentes reais):
E2E_EMAIL='owner@example.com' E2E_PASSWORD='senha-do-dashboard' E2E_DESTRUCTIVE=1 pnpm test:e2e

# Apontando para outro host:
E2E_BASE_URL=https://staging.exemplo.com E2E_EMAIL='owner@example.com' E2E_PASSWORD='...' pnpm test:e2e
...
| `E2E_EMAIL`       | _(unset)_                | Email do dashboard quando o launcher usa login por email+senha.                              |
...
1. Faz `POST /api/auth/login` usando `E2E_PASSWORD`.
   Quando `E2E_EMAIL` existe, envia `{email,password}`; sem ele, mantém o
   formato legado `{password}`.
