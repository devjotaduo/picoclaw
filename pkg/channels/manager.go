...
func (m *Manager) SetupHTTPServerListeners(listeners []net.Listener, addr string, healthServer *health.Server) {
	...
	// WriteTimeout must accommodate long-lived webhook responses such as
	// streaming channel handlers. Agent replies can take 30-180s when the
	// LLM provider has a cold start or tool-call iteration, so keep this
	// above the longest realistic turn while still reaping zombie clients
	// that hold sockets open indefinitely.
	//
