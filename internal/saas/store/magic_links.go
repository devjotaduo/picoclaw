...
func (s *MagicLinkStore) Insert(ctx context.Context, m *MagicLink) error {
	const q = `
		INSERT INTO magic_links (nonce, tenant_id, expires_at)
		VALUES ($1, $2, $3)
	`
	_, err := s.DB.Pool.Exec(ctx, q, m.Nonce, m.TenantID, m.ExpiresAt)
	return err
}

func (s *MagicLinkStore) Get(ctx context.Context, nonce string) (*MagicLink, error) {
	const q = `
		SELECT nonce, tenant_id, created_at, expires_at, consumed_at, summary
		FROM magic_links WHERE nonce = $1
	`
	m := &MagicLink{}
	err := s.DB.Pool.QueryRow(ctx, q, nonce).Scan(
		&m.Nonce, &m.TenantID, &m.CreatedAt, &m.ExpiresAt, &m.ConsumedAt, &m.Summary,
	)
...
func (s *MagicLinkStore) ListByTenant(ctx context.Context, tenantID string, limit int) ([]*MagicLink, error) {
	...
	for rows.Next() {
		m := &MagicLink{}
		if err := rows.Scan(&m.Nonce, &m.TenantID, &m.CreatedAt, &m.ExpiresAt, &m.ConsumedAt, &m.Summary); err != nil {
			return nil, err
		}
		out = append(out, m)
	}
