...
// that the upload silently drops. These are either build artefacts
...
// is a runtime / build / cache artefact that the upload should silently
...
func validateWorkspaceZip(zr *zip.Reader) error {
	...
	for _, f := range zr.File {
		...
		// Runtime / build / cache artefacts are silently dropped. The
