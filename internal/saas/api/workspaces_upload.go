...
// frontendSrcSkipPrefixes lists the top-level dirs under frontend-src/
// that the upload silently drops. These are either build artifacts
// (regenerated by `pnpm vite build`) or dependency caches that the
// operator should rehydrate via `pnpm install` on the workspace —
// shipping them in the zip blows the 50 MiB upload cap with a
// node_modules of any non-trivial size and there's no value in keeping
// the snapshot in the workspace template.
//
// The list is intentionally small and exact-match (no wildcards) so a
// future Vite plugin emitting some clever dir name doesn't get silently
// stripped. Reviewed against the Vite/Next/Nuxt/SvelteKit/Astro defaults
// as of 2026-05.
var frontendSrcSkipPrefixes = []string{
	"node_modules",
	".cache",
	".git",
	"dist",        // local vite output (we have a dedicated frontend-dist/ dir)
	"build",       // CRA / generic build output
	".next",       // Next.js
	".nuxt",       // Nuxt
	".output",     // Nuxt 3 / Nitro
...
// shouldSkipUploadEntry returns true when the given workspace-relative
// path (e.g. "home/workspace/sessions/x.json" or "frontend-src/node_modules/foo")
// is a runtime / build / cache artifact that the upload should silently
// drop instead of writing to disk.
//
//   - home/<...>:         delegates to shouldHideTenantPath (sessions/,
//     whatsapp/, state/, *.pid, dashboardauth.db, ...)
//   - frontend-src/<...>: drops node_modules/, .cache/, .git/, dist/, etc.
//   - frontend-dist/<...>: NEVER skipped — that subtree is literally the
//     compiled bundle the launcher serves.
func shouldSkipUploadEntry(cleaned string, isDir bool) bool {
	if homeRel, isHome := strings.CutPrefix(cleaned, tenant.WorkspaceHomeSubdir+"/"); isHome {
		return shouldHideTenantPath(homeRel, isDir)
	}
	if srcRel, isSrc := strings.CutPrefix(cleaned, tenant.WorkspaceFrontendSrcSubdir+"/"); isSrc {
		for _, p := range frontendSrcSkipPrefixes {
			if srcRel == p || strings.HasPrefix(srcRel, p+"/") {
				return true
			}
		}
	}
...
func validateWorkspaceZip(zr *zip.Reader) error {
	...
	for _, f := range zr.File {
		...
		// Runtime / build / cache artifacts are silently dropped. The
		// per-subtree policy is in shouldSkipUploadEntry: home/ uses the
		// tenant-files hide list (sessions/, whatsapp/, ...); frontend-src/
		// drops node_modules + framework caches; frontend-dist/ keeps
		// everything (it IS the build output).
		if shouldSkipUploadEntry(cleaned, f.FileInfo().IsDir()) {
			continue
		}
...
func extractWorkspaceZip(zr *zip.Reader, dst string) error {
	...
	for _, f := range zr.File {
		...
		cleaned := filepath.ToSlash(filepath.Clean(filepath.FromSlash(rel)))
		// Same skip policy as validator — see shouldSkipUploadEntry.
		if shouldSkipUploadEntry(cleaned, f.FileInfo().IsDir()) {
			continue
		}
