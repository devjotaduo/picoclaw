...
def score_bant(payload: dict) -> dict:
    fields = ["budget", "authority", "need", "timeline"]
    score = sum(25 for field in fields if payload.get(field))
    missing = [field for field in fields if not payload.get(field)]
    qualified = score >= 70
    return {"score": score, "qualified": qualified, "missing_fields": missing, "handoff_agent": "agente-vendedor" if qualified else "agente-recepcionista"}


def main() -> int:
    parser = argparse.ArgumentParser(description="Score a lead using deterministic BANT fields.")
    parser.parse_args()
    try:
        data = read_json_stdin()
        reject_sensitive_keys(data)
        write_json_stdout(score_bant(data))
        return 0
