...
8. Encerramento: feche o discovery com a skill `onboarding-state`
   usando a ação `discovery_close`. Ela grava `memory/empresa.md` e marca
   o discovery como concluído numa única operação.
...
- `onboarding-state` (state machine — init, discovery_close, mark_*, get)
